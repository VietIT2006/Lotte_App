...
## HƯỚNG DẪN KẾT NỐI VÀ CHẠY DỰ ÁN (MÁY ẢO & MÁY THẬT)

Dự án hỗ trợ tính năng **Tự động nhận diện & cấu hình kết nối mạng (Auto IP Discovery)** giúp việc kết nối ứng dụng Android với Localhost Backend trở nên cực kỳ đơn giản mà không cần chỉnh sửa địa chỉ IP trong mã nguồn.

### 1. Đối với Máy ảo (Emulator)
*   Chỉ cần cắm và chạy máy ảo (như Pixel 7, Medium Phone...).
*   Nhấn nút **Run** trên Android Studio.
*   Ứng dụng sẽ tự động kết nối tới Backend thông qua cổng mặc định `http://10.0.2.2:3000`.

### 2. Đối với Máy thật (Physical Device) - Kết nối chung Wi-Fi
*   **Điều kiện:** Điện thoại và máy tính chạy backend của bạn phải **kết nối chung một mạng Wi-Fi**.
*   Khi ứng dụng Android vừa khởi động trên máy thật, nó sẽ tự động chạy tiến trình quét nhanh dải mạng LAN nội bộ (qua giao thức Wi-Fi) để tìm máy tính đang chạy Backend ở cổng `3000`.
*   Ứng dụng sẽ tự động lấy IP máy tính đó làm `BASE_URL` và kết nối ngay lập tức mà không cần bất kỳ cấu hình nào.

### 3. Đối với Máy thật - Kết nối qua cáp USB (Hoặc ADB Không dây)
Nếu máy thật không chung Wi-Fi hoặc bạn muốn chạy trực tiếp bằng dây cáp:
1. Cắm cáp USB kết nối điện thoại vào máy tính (hoặc kết nối qua ADB không dây).
2. Đảm bảo điện thoại đã kích hoạt **USB Debugging** (Gỡ lỗi USB) trong Cài đặt nhà phát triển.
3. Mở **Terminal** trong Android Studio và chạy lệnh sau để chuyển tiếp cổng:
   ```bash
   adb reverse tcp:3000 tcp:3000
   ```
4. Khi đã chạy lệnh này, ứng dụng Android trên máy thật sẽ kết nối trực tiếp với localhost của máy tính thông qua cáp USB và tải dữ liệu bình thường.
